...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"English-App" <no-reply@accounts.english-app.herokuapp.com>',
        //receiver access
        to,
        //subject
        subject,
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Mã xác nhận đăng ký tài khoản cho website English - App của bạn.<br />
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        English - App đã nhận được yêu cầu lấy lại mật khẩu từ bạn.<br />
