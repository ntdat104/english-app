...
exports.postLogin = async (req, res) => {
  try {
    ...
    // check password
    const isMatch = await bcryptService.compare(password, account.password);
    if (!isMatch) {
      return res.status(401).json({ message: 'Mật khẩu không đúng' });
    }

    // set cookie with jwt
    const token = await jwtService.signAccessToken({ accountId: account._id });
...
exports.postLoginSocialNetwork = async (req, res) => {
  try {
    ...
    // set cookie with jwt
    const token = await jwtService.signAccessToken({ accountId });
